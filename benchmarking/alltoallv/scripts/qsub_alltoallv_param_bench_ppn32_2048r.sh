#!/bin/bash -l
#PBS -N alltoallv_param_bench_2048r
...
echo "Running Alltoallv benchmark: algorithms=${ALLTOALLV_PARAM_ALGORITHMS} total_ranks=${TOTAL_RANKS} ppn=${RANKS_PER_NODE} warmup=${WARMUP_ROUNDS} actual=${MEASURED_ROUNDS} max_msg_size=${MAX_MSG_SIZE} hieata_radices=${ALLTOALLV_HIEATA_RADICES} hieata_bthsizes=${ALLTOALLV_HIEATA_BTHSIZES} parata_radices=${ALLTOALLV_PARATA_RADICES}"

rm -f "$OUTPUT_CSV"

"$MPIEXEC" --pmi=cray -n "$TOTAL_RANKS" --ppn "$RANKS_PER_NODE" ./alltoallv_param_bench \
