...
for ALGORITHM in "${BCAST_ALGORITHMS[@]}"; do
    ...
    echo "Running Bcast benchmark: algorithm=${ALGORITHM} total_ranks=${TOTAL_RANKS} ppn=${RANKS_PER_NODE} warmup=${WARMUP_ROUNDS} actual=${MEASURED_ROUNDS} max_msg_size=${MAX_MSG_SIZE}"
...
echo "Finished Bcast benchmark sweep. Timing/counter CSV path template: ${OUTPUT_CSV%.csv}_<algorithm-output-label>.csv"
