...
JOB_TAG="${JOB_TAG%%.*}"
OUTPUT_CSV="${BENCH_DIR}/bcast_bench_short_ppn32_${TOTAL_RANKS}r_${JOB_TAG}.csv"

cd "$BENCH_DIR"

"$MPICH_PREFIX/bin/mpicxx" -std=c++11 -O2 bcast_bench_short.cpp -o bcast_bench_short
...
for ALGORITHM in "${BCAST_ALGORITHMS[@]}"; do
    ...
    echo "Running short Bcast benchmark: algorithm=${ALGORITHM} total_ranks=${TOTAL_RANKS} ppn=${RANKS_PER_NODE} warmup=${WARMUP_ROUNDS} actual=${MEASURED_ROUNDS} max_msg_size=${MAX_MSG_SIZE}"

    rm -f "$ALGORITHM_OUTPUT_CSV" "$ALGORITHM_COUNTER_CSV"

    "$MPIEXEC" --pmi=cray -n "$TOTAL_RANKS" --ppn "$RANKS_PER_NODE" ./bcast_bench_short \
...
echo "Wrote one short timing CSV and one counter CSV per algorithm under: ${OUTPUT_CSV%.csv}_<algorithm-output-label>.csv"
