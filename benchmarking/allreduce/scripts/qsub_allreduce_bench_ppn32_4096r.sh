...
echo "Running Allreduce benchmark: algorithms=${ALLREDUCE_ALGORITHMS} total_ranks=${TOTAL_RANKS} ppn=${RANKS_PER_NODE} warmup=${WARMUP_ROUNDS} actual=${MEASURED_ROUNDS} max_msg_size=${MAX_MSG_SIZE}"

rm -f "$OUTPUT_CSV"

"$MPIEXEC" --pmi=cray -n "$TOTAL_RANKS" --ppn "$RANKS_PER_NODE" ./allreduce_bench \
